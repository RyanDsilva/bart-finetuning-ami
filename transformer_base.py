...
class BaseTransformer(pl.LightningModule):
    ...
    def get_tqdm_dict(self):
        tqdm_dict = {"loss": "{:.3f}".format(
            self.trainer.batch_loss_value.mean()), "lr": self.lr_scheduler.get_last_lr()[-1]}
        return tqdm_dict
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--adam_epsilon", default=1e-9,
                            type=float, help="Epsilon for Adam optimizer.")
        parser.add_argument("--warmup_steps", default=1300,
...
def generic_train(model, args):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filepath=args.output_dir, prefix="ami", monitor="val_loss", mode="min", save_top_k=2
    )
